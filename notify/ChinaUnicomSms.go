...
func (srv *ChinaUnicomSms) Send() (aliSmsResult *dysmsapi20170525.SendBatchSmsResponse, err error) {
	umsConfig := global.SERVER_CONFIG.UmsConfig
	if !umsConfig.Enable {
		fmt.Println("ChinaUnicomSms not enable !")
		return nil, nil
	}
	templateCode := umsConfig.TemplateCode
	if len(srv.PhoneNumberJson) == 0 {
		srv.PhoneNumberJson = umsConfig.ToMobiles
	}
	params, serialNumber, err := srv.buildParams(templateCode, umsConfig)
	if err != nil {
		return
	}

	resp, err := srv.send(umsConfig.Url, params, time.Duration(10)*time.Second)
	if err != nil {
